...
PLATO_INPUT_BLOCK_STRUCT((plato)(input_parser),
                         fruits, plato::components::ComponentType::kGeometry,
...
PLATO_NAMED_INPUT_BLOCK_STRUCT((plato)(input_parser),
                         vegetables, plato::components::ComponentType::kObjective,
                         (bool, tomato, "Is it a fruit?")
                         (unsigned int, potato, "Boil 'em, mash 'em, stick 'em in a stew."))

PLATO_INPUT_BLOCK_STRUCT((plato)(input_parser),
                         rocks, plato::components::ComponentType::kFilter,
                         (int, shininess, ""))
...
const auto kComponentParsers = std::unordered_map<std::string, ComponentBlockParser>{
    {"fruits", ComponentBlockParser{fruits{}, ComponentTypeHelper<components::ComponentType::kGeometry>{}}},
    {"vegetables", ComponentBlockParser{vegetables{}, ComponentTypeHelper<components::ComponentType::kObjective>{}}}};

void check_component_type(const std::vector<InputDataBlock>& aInputsForComponent,
                          const components::ComponentType aComponentType,
...
TEST(ParsedInput, ParsesFullValidInput)
{
    ...
    const auto& tVegetableInputs = tParsedInput.get<components::ComponentType::kObjective>();
    ASSERT_EQ(tVegetableInputs.size(), 1U);
    EXPECT_EQ(tVegetableInputs.front().mBlockName, "vegetables");
    EXPECT_EQ(tVegetableInputs.front().mComponentType, components::ComponentType::kObjective);
    ...
    const auto& tFruitInputs = tParsedInput.get<components::ComponentType::kGeometry>();
    ASSERT_EQ(tFruitInputs.size(), 1U);
    EXPECT_EQ(tFruitInputs.front().mBlockName, "fruits");
    EXPECT_EQ(tFruitInputs.front().mComponentType, components::ComponentType::kGeometry);
...
TEST(ParsedInput, OneComponent)
{
    const auto tGeometry =
        InputDataBlock{/*.mComponentType=*/components::ComponentType::kGeometry, /*.mBlockName=*/"geo_block", {}};
    const auto tParsedInput = ParsedInput{{tGeometry}};

    const auto& tResult = tParsedInput.get<components::ComponentType::kGeometry>();
    ASSERT_EQ(tResult.size(), 1U);
    check_component_type(tResult, components::ComponentType::kGeometry, TEST_CONTEXT("Geometry component type"));
    check_block_names(tResult, {"geo_block"}, TEST_CONTEXT("Geometry block name"));

    EXPECT_TRUE(tParsedInput.get<components::ComponentType::kConstraint>().empty());
    EXPECT_TRUE(tParsedInput.get<components::ComponentType::kFilter>().empty());
    EXPECT_TRUE(tParsedInput.get<components::ComponentType::kObjective>().empty());
    EXPECT_TRUE(tParsedInput.get<components::ComponentType::kProcessManager>().empty());
}
...
TEST(ParsedInput, TwoComponents)
{
    const auto tGeometry =
        InputDataBlock{/*.mComponentType=*/components::ComponentType::kGeometry, /*.mBlockName=*/"geo_block", {}};
    const auto tFilter =
        InputDataBlock{/*.mComponentType=*/components::ComponentType::kFilter, /*.mBlockName=*/"filter_block", {}};
    const auto tParsedInput = ParsedInput{{tFilter, tGeometry}};

    const auto& tGeometryResults = tParsedInput.get<components::ComponentType::kGeometry>();
    ASSERT_EQ(tGeometryResults.size(), 1U);
    check_component_type(tGeometryResults, components::ComponentType::kGeometry,
                         TEST_CONTEXT("Geometry component type"));
    check_block_names(tGeometryResults, {"geo_block"}, TEST_CONTEXT("Geometry block name"));

    const auto& tFilterResults = tParsedInput.get<components::ComponentType::kFilter>();
    ASSERT_EQ(tFilterResults.size(), 1U);
    check_component_type(tFilterResults, components::ComponentType::kFilter, TEST_CONTEXT("Filter component type"));
...
{
    const auto tConstraint1 = InputDataBlock{
        .mComponentType = components::ComponentType::kConstraint, .mBlockName = "constraint_block_1", .mInput = {}};
    const auto tConstraint2 = InputDataBlock{
        .mComponentType = components::ComponentType::kConstraint, .mBlockName = "constraint_block_2", .mInput = {}};
    const auto tObjective1 = InputDataBlock{
        .mComponentType = components::ComponentType::kObjective, .mBlockName = "objective_block_1", .mInput = {}};
    const auto tObjective2 = InputDataBlock{
        .mComponentType = components::ComponentType::kObjective, .mBlockName = "objective_block_2", .mInput = {}};
    const auto tProcessManager = InputDataBlock{.mComponentType = components::ComponentType::kProcessManager,
                                                .mBlockName = "process_manager_block",
                                                .mInput = {}};

    const auto tParsedInput = ParsedInput{{tConstraint1, tProcessManager, tObjective1, tConstraint2, tObjective2}};

    const auto& tObjectiveResults = tParsedInput.get<components::ComponentType::kObjective>();
    ASSERT_EQ(tObjectiveResults.size(), 2U);
    check_component_type(tObjectiveResults, components::ComponentType::kObjective,
                         TEST_CONTEXT("Objective component types"));
    check_block_names(tObjectiveResults, {"objective_block_1", "objective_block_2"},
                      TEST_CONTEXT("Objective block names"));

    const auto& tConstraintResults = tParsedInput.get<components::ComponentType::kConstraint>();
    ASSERT_EQ(tConstraintResults.size(), 2U);
    check_component_type(tConstraintResults, components::ComponentType::kConstraint,
                         TEST_CONTEXT("Constraint component types"));
    check_block_names(tConstraintResults, {"constraint_block_1", "constraint_block_2"},
                      TEST_CONTEXT("Constraint block names"));

    const auto& tProcessManagerResults = tParsedInput.get<components::ComponentType::kProcessManager>();
    ASSERT_EQ(tProcessManagerResults.size(), 1U);
    check_component_type(tProcessManagerResults, components::ComponentType::kProcessManager,
    ...
    EXPECT_TRUE(tParsedInput.get<components::ComponentType::kGeometry>().empty());
    EXPECT_TRUE(tParsedInput.get<components::ComponentType::kFilter>().empty());
}
...
TEST(ParsedInput, GetInputBlockWithType)
{
    ...
    const auto tObjective1 = InputDataBlock{.mComponentType = components::ComponentType::kObjective,
                                            .mBlockName = "vegetables",
                                            .mInput = InputBlockWrapper{tObjectiveData1}};

    const auto tObjectiveData2 =
        input_parser::vegetables{/*.name=*/std::string{"objective 2"}, /*.tomato=*/false, /*.potato=*/11U};
    const auto tObjective2 = InputDataBlock{.mComponentType = components::ComponentType::kObjective,
                                            .mBlockName = "vegetables",
                                            .mInput = InputBlockWrapper{tObjectiveData2}};

    const auto tGeometryData = input_parser::fruits{/*.banana=*/42, /*.apple=*/13.0};
    const auto tGeometry = InputDataBlock{.mComponentType = components::ComponentType::kGeometry,
                                          .mBlockName = "fruits",
                                          .mInput = InputBlockWrapper{tGeometryData}};
