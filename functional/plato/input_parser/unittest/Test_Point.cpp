...
TEST(Point, ParsingTests)
{
    {
        const auto tInput = std::string_view{" (1.0, 1.0, 2.0) "};
        const auto tGold = input_parser::Point{1.0, 1.0, 2.0};
        input_parser::test_utilities::expect_valid_input(tInput, tGold, TEST_CONTEXT("Simple valid Point"));
    }
    {
        const auto tInput = std::string_view{"1.0e-1,1.0e-2,2.0)"};
        input_parser::test_utilities::expect_invalid_input<input_parser::Point>(
