...
ReturnType accumulate_function_on_tesselation_impl(const std::vector<common::Coordinate>& aCoordinates,
                                                   const F& aFunction)
{
...
common::Coordinate centroid_impl(const std::vector<common::Coordinate>& aCoordinates)
{
    ...
    assert(tVolume != 0);
    const common::Coordinate tVolumeWeightedCentroid =
        accumulate_function_on_tesselation_impl<STK_TOPOLOGY, common::Coordinate>(
            aCoordinates, [](const typename TesselationTraits<STK_TOPOLOGY>::ElementType& aReturnObject)
