...
TEST(LinearMaskBuilder, GenerateDistanceMapNodal)
{
    ...
    const auto tLinearMaskBuilder = test_utilities::create_simple_linear_mask_builder();
...
TEST(LinearMaskBuilderDetail, DistributeSearchVectorsAndStkSearch)
{
    ...
    const auto tSearchResults = distribute_search_vectors_and_stk_search_with_spheres(
        TargetRowVector{tCoordinates}, SourceColumnVector{tCoordinates}, tSearchRadius, tCommunicator);
