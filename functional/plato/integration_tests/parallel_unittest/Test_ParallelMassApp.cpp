...
TEST(ParallelMassObjective, CallValueAndGradient)
{
    ...
    const double tMass = tSharedLib.f(design_variables::MeshDesignVariables{tRankMeshName, {}});
    EXPECT_DOUBLE_EQ(tMass, kMeshGenerator.volume());

    const auto tGrad = tSharedLib.df(design_variables::MeshDesignVariables{tRankMeshName, {}});
