...
PLATO_GEOMETRY_INPUT_BLOCK_STRUCT(
    (plato)(input_parser), new_density_topology,
...
[[nodiscard]] auto create_valid_density_topology_geometry_input() -> input_parser::new_density_topology;
...
[[nodiscard]] auto validate_initial_density_value(const input_parser::new_density_topology& aInput)
    -> std::optional<std::string>;

/// @brief Validates that exactly one specifier for the intitial topology is used, the `initial_density_value` or
/// `initial_field_name`
[[nodiscard]] auto validate_exactly_one_initial_topology_specifier(const input_parser::new_density_topology& aInput);
