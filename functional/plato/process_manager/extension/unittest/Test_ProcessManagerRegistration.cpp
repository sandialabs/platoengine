...
TEST(ProcessManagerRegistration, ProcessManagerInput)
{
    ...
    static_assert(std::variant_size_v<TestInput> == 5);
    ...
    static_assert(std::is_same_v<std::variant_alternative_t<4, TestInput>, input_parser::snopt_optimization>);
}
...
TEST(ProcessManagerRegistration, ValidatedProcessManagerInputVector)
{
    ...
    static_assert(std::is_same_v<std::variant_alternative_t<4, TestInput>,
                                 core::ValidatedInputTypeWrapper<input_parser::snopt_optimization>>);
}
}  // namespace plato::process_manager::library::unittest
