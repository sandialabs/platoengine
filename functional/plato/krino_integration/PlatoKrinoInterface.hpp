...
class PlatoKrinoInterface
{
   ...
   private:
    ...
    void generate_bounding_box_mesh(krino::BoundingBoxMesh &bboxMesh,
    ...
    void initialize_levelset_field_for_plane(const stk::mesh::BulkData &mesh,
                                             krino::FieldRef levelSetField,
                                             const stk::math::Vector3d &normal,
                                             const double offset);
    void initialize_levelset_field_for_spheres(const stk::mesh::BulkData &mesh,
                                               krino::FieldRef levelSetField,
                                               const std::vector<std::pair<stk::math::Vector3d, double>> &spheres);
    void initialize_levelset_fields_from_primitives(const stk::mesh::BulkData &mesh,
                                                    krino::FieldRef levelSetField,
                                                    const LevelsetPrimitives &aLevelsetPrimitives);
    void decompose_mesh_to_conform_to_levelsets(stk::mesh::BulkData &mesh,
                                                const std::vector<krino::LS_Field> &lsFields);
    std::map<stk::mesh::EntityId, InterfaceNode_DXDP> get_levelset_shape_sensitivities(
        const stk::mesh::BulkData &mesh, const krino::FieldRef levelSetField);
    void fill_node_ids_for_nodes(const stk::mesh::BulkData &mesh,
                                 const std::vector<stk::mesh::Entity> &parentNodes,
                                 std::vector<stk::mesh::EntityId> &parentNodeIds);
    void fill_d_coords_d_levelsets(
