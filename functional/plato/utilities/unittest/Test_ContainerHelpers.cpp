...
TEST(ContainerHelpers, FindKeyWithValue)
{
    ...
    ASSERT_FALSE(find_key_with_value(tMap, "42"));

    ASSERT_TRUE(find_key_with_value(tMap, "0"));
    EXPECT_EQ(find_key_with_value(tMap, "0").value(), 0);

    ASSERT_TRUE(find_key_with_value(tMap, "5"));
    EXPECT_EQ(find_key_with_value(tMap, "5").value(), 5);

    ASSERT_TRUE(find_key_with_value(tMap, "10"));
    EXPECT_EQ(find_key_with_value(tMap, "10").value(), 10);
}
