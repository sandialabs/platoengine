...
TEST(TupleHelpers, MakeCompatibleTuple)
{
    auto tTuple1 = std::tuple<int, double, bool>{1, 42.0, true};
    auto tTuple2 = make_compatible_tuple<std::tuple<double>>(tTuple1);

    using Tuple2Type = decltype(tTuple2);
    static_assert(std::tuple_size_v<Tuple2Type> == 1);
    static_assert(std::is_same_v<std::tuple_element_t<0, Tuple2Type>, double>);

    EXPECT_EQ(std::get<0>(tTuple2), 42.0);
}

TEST(TupleHelpers, MakeCompatibleTupleIdentity)
{
    using Tuple = std::tuple<int, double>;
    auto tTuple1 = Tuple{1, 42.0};
    auto tTuple2 = make_compatible_tuple<Tuple>(tTuple1);

    EXPECT_EQ(std::get<0>(tTuple2), 1);
    EXPECT_EQ(std::get<1>(tTuple2), 42.0);
}

TEST(TupleHelpers, MakeCompatibleTupleSwapEntries)
{
    auto tTuple1 = std::tuple<int, double>{1, 42.0};
    auto tTuple2 = make_compatible_tuple<std::tuple<double, int>>(tTuple1);

    EXPECT_EQ(std::get<0>(tTuple2), 42.0);
    EXPECT_EQ(std::get<1>(tTuple2), 1);
}

TEST(TupleHelpers, MakeCompatibleTupleDegenerate)
{
    auto tTuple1 = std::tuple<int>{42};
    auto tTuple2 = make_compatible_tuple<std::tuple<>>(tTuple1);
    using Tuple2Type = decltype(tTuple2);
    static_assert(std::tuple_size_v<Tuple2Type> == 0);
}

TEST(TupleHelpers, MakeCompatibleTupleCopyCounts)
{
    const auto tTuple1 = std::tuple<CopyCounter, double>{};
    const auto tTuple2 = make_compatible_tuple<std::tuple<CopyCounter>>(tTuple1);
    EXPECT_EQ(std::get<0>(tTuple2).mCopies, 1);
    EXPECT_EQ(std::get<0>(tTuple2).mMoves, 1);
}

TEST(TupleHelpers, MakeCompatibleTupleMoveCounts)
{
    auto tTuple1 = std::tuple<CopyCounter, double>{};
    const auto tTuple2 = make_compatible_tuple<std::tuple<CopyCounter>>(std::move(tTuple1));
    EXPECT_EQ(std::get<0>(tTuple2).mCopies, 0);
    EXPECT_EQ(std::get<0>(tTuple2).mMoves, 2);
}
