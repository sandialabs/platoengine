...
template <typename T> 
T getOrThrow(const Plato::InputData &aInputData,
...
SpherePatternData readSpherePatternData(const Plato::InputData &aNode)
{
    ...
    tData.mCoordMins = {getOrThrow<double>(aNode, "bbox_xmin", "ERROR: Levelset definition bounding box xmin was not specified."),
    ...
    tData.mCoordMaxes = {getOrThrow<double>(aNode, "bbox_xmax", "ERROR: Levelset definition bounding box xmax was not specified."),
    ...
    tData.mNumSpheres = {getOrThrow<int>(aNode, "num_spheres_x", "ERROR: Levelset definition number of spheres in the x direction was not specified."),
                   getOrThrow<int>(aNode, "num_spheres_y", "ERROR: Levelset definition number of spheres in the y direction was not specified."),
                   getOrThrow<int>(aNode, "num_spheres_z", "ERROR: Levelset definition number of spheres in the z direction was not specified.")};
    tData.mSphereRadius = getOrThrow<double>(aNode, "radius", "ERROR: Levelset definition sphere radius was not specified.");
    tData.mSpheresCanOverlapBoundingBox = getOrThrow<bool>(aNode, "overlap_bbox", "ERROR: Levelset definition overlap param not specified.");
    return tData;
}

Sphere readSphereData(const Plato::InputData &aNode)
{
    double tCenterX = getOrThrow<double>(aNode, "center_x", "ERROR: Levelset definition sphere center_x was not specified.");
    double tCenterY = getOrThrow<double>(aNode, "center_y", "ERROR: Levelset definition sphere center_y was not specified.");
    double tCenterZ = getOrThrow<double>(aNode, "center_z", "ERROR: Levelset definition sphere center_z was not specified.");
...
{
    double tNormalX = getOrThrow<double>(aNode, "normal_x", "ERROR: Levelset definition plane normal_x was not specified.");
    double tNormalY = getOrThrow<double>(aNode, "normal_y", "ERROR: Levelset definition plane normal_y was not specified.");
    double tNormalZ = getOrThrow<double>(aNode, "normal_z", "ERROR: Levelset definition plane normal_z was not specified.");
...
{
    std::pair<double,double> tStartAndSpacing(aPatternData.mCoordMins[aDimension],
...
{
    const double tSpacing = (aPatternData.mCoordMaxes[aDimension]-aPatternData.mCoordMins[aDimension])/(aPatternData.mNumSpheres[aDimension]-1);
...
{
    std::pair<double,double> tStartAndSpacing(aPatternData.mCoordMins[aDimension], (aPatternData.mCoordMaxes[aDimension]-
                                              aPatternData.mCoordMins[aDimension])/(aPatternData.mNumSpheres[aDimension]+1));
    return tStartAndSpacing;
}
...
SphereLocatorData calculateSphereStartsAndSpacing(const SpherePatternData &aData)
{
    ...
    for(size_t tCurDimension=0; tCurDimension<tNumDimensions; ++tCurDimension)
    {
        if(aData.mSpheresCanOverlapBoundingBox)
        {
            ...
            {
                tLocatorData.mStartAndSpacing[tCurDimension] = calculateOverlappingSingleSphereLocatorData(aData, tCurDimension);
            }
            else
            {
                tLocatorData.mStartAndSpacing[tCurDimension] = calculateOverlappingManySphereLocatorData(aData, tCurDimension);
            }
        }
        else
        {
            tLocatorData.mStartAndSpacing[tCurDimension] = calculateNonOverlappingSphereLocatorData(aData, tCurDimension);
        }
    }
    return tLocatorData;
}
...
std::vector<Sphere> generateSpheres(const SpherePatternData &aData)
{
    ...
    {
        const double tSphereCenterX = tLocatorData.mStartAndSpacing[0].first + (i+1)*tLocatorData.mStartAndSpacing[0].second;
        for(int j=0; j<aData.mNumSpheres[1]; ++j)
        {
            const double tSphereCenterY = tLocatorData.mStartAndSpacing[1].first + (j+1)*tLocatorData.mStartAndSpacing[1].second;
            for(int k=0; k<aData.mNumSpheres[2]; ++k)
            {
                const double tSphereCenterZ = tLocatorData.mStartAndSpacing[2].first + (k+1)*tLocatorData.mStartAndSpacing[2].second;
